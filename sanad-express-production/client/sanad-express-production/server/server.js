...
// تحديد كود محطة الفرز والتوجيه الجغرافي لسند إكسبريس
...
// إنشاء طلب جديد بنظام سند إكسبريس (SND-XXXXXX)
app.post('/api/orders', (req, res) => {
  const { branchId, customerName, customerPhone, customerAddress, items, totalAmount, paymentMethod, notes, customerCoords, driverCommission, assignedDriverId } = req.body;
  ...
  const newOrder = {
    ...
    items: (items && items.length > 0) ? items : [{ name: 'شحنة منتجات سند إكسبريس', qty: 1, price: Number(totalAmount) || 150 }],
...
app.get('/api/financials', (req, res) => {
  const totalCashOnHandDrivers = drivers.reduce((acc, d) => acc + (d.cashOnHand || 0), 0);
  const totalCommissionsEarned = drivers.reduce((acc, d) => acc + (d.totalCommissionToday || 0), 0);
...
app.get('/api/track/:trackingNumber', (req, res) => {
  ...
  const timeline = [
    ...
    {
      title: 'تم الاستلام بواسطة سند إكسبريس',
...
// 5. محطة التجهيز والمستودع الذكي (رحلة التجهيز لسند إكسبريس: تحضير -> تعبئة -> تشييك -> تغليف)
...
let inventoryItems = [
  ...
  { sku: 'PKG-301', name: 'كراتين شحن سند إكسبريس مقاس 1', category: 'مواد تغليف', branchId: 'branch-iklil-dammam', branchName: 'فرع إكليل الدمام', qty: 450, minQty: 100, unitPrice: 3.50, shelfLocation: 'مستودع 1' },
...
let managers = [
  { id: 'mgr-1', name: 'م. ياسين المختار', phone: '+966501122334', email: 'yaseen@sanadexpress.com', role: 'مدير عام العمليات (Super Admin)', branchName: 'كافة الفروع', status: 'نشط', lastActive: 'الآن' },
  { id: 'mgr-2', name: 'تركي الحربي', phone: '+966504433221', email: 'turki.dammam@sanadexpress.com', role: 'مشرف فرع الدمام', branchName: 'فرع إكليل الدمام', status: 'نشط', lastActive: 'منذ 5 دقائق' },
  { id: 'mgr-3', name: 'خالد الدوسري', phone: '+966558899001', email: 'khaled.vape@sanadexpress.com', role: 'مشرف متجر فيب الشرق', branchName: 'متجر فيب الشرق', status: 'نشط', lastActive: 'منذ 12 دقيقة' },
  { id: 'mgr-4', name: 'ماجد الشمري', phone: '+966567788112', email: 'majed.jubail@sanadexpress.com', role: 'مشرف فرع إكليل الجبيل', branchName: 'فرع إكليل الجبيل', status: 'نشط', lastActive: 'منذ 30 دقيقة' },
  { id: 'mgr-5', name: 'ريان الزهراني', phone: '+966541199882', email: 'rayan.wh@sanadexpress.com', role: 'مسؤول الفرز والتجهيز', branchName: 'مستودع الشرقية المركزي', status: 'نشط', lastActive: 'الآن' }
];
...
let equipment = [
  ...
  { id: 'EQ-03', assetType: 'حقيبة حرارية عازلة سند إكسبريس مقاس كبير', serialNumber: 'BAG-EXP-04', driverName: 'عبدالرحمن الناصر', branchName: 'متجر إكليل فيب', assignedDate: '25/08/2026', status: 'بالخدمة' },
